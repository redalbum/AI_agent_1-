...
net session >nul 2>&1
if %errorLevel% neq 0 (
    echo ⚠️  Требуются права администратора для установки Python
...
if %errorLevel% neq 0 (
    ...
    powershell -Command "$hash = (Get-FileHash '%TEMP%\python_installer.exe' -Algorithm SHA256).Hash; if ($hash -ne '6EB2C88D05BB58B4A990EAB27A2F6A09E0FCBF1FCEA2906E3C43D55E4D1ED81B') { Remove-Item '%TEMP%\python_installer.exe'; Write-Error 'Ошибка: контрольная сумма Python-установщика не совпадает!'; exit 1 }"
    if %errorLevel% neq 0 (
...
cd /d "%~dp0"
if exist "venv" (
    echo        ⚠️  venv уже существует, пропускаю...
) else (
    python -m venv venv
    echo        ✅ venv создан
)
...
if exist ".env" (
    echo        ⚠️  .env уже существует, не перезаписываю
) else (
...
powershell -Command "$ws = New-Object -ComObject WScript.Shell; $s = $ws.CreateShortcut('%DESKTOP%\⚙️ Настройки бота.lnk'); $s.TargetPath = 'notepad.exe'; $s.Arguments = '%SCRIPT_DIR%.env'; $s.WorkingDirectory = '%SCRIPT_DIR%'; $s.Save()"
powershell -Command "$ws = New-Object -ComObject WScript.Shell; $s = $ws.CreateShortcut('%DESKTOP%\🚀 Запустить бота.lnk'); $s.TargetPath = '%SCRIPT_DIR%START_BOT.bat'; $s.WorkingDirectory = '%SCRIPT_DIR%'; $s.Save()"
...
echo    4. Заполните ONEC_BASE_PATH — путь к папке базы 1С
echo    5. Заполните OPENROUTER_API_KEY (от openrouter.ai)
