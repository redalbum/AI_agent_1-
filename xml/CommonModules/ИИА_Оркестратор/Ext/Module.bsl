...
Функция ПодготовитьDSLПередИсполнением(DSLДляИсполнения, СообщениеОшибки = "") Экспорт
...
Функция ВыполнитьСтадиюExecute(СсылкаДиалога) Экспорт
	...
	СообщениеPrecheck = "";
	DSLПодготовленный = ПодготовитьDSLПередИсполнением(DSLДляИсполнения, СообщениеPrecheck);
	Если ПустаяСтрока(DSLПодготовленный) Тогда
		РезультатPrecheck = Новый Структура("Успех,Сообщение,error_code,error_context", Ложь, "Ошибка pre-execution DSL check: " + СообщениеPrecheck, "invalid_step", Новый Структура("stage,reason", "Execute", "pre_execution_schema_check"));
		ИИА_Сервер.СохранитьРезультатExecutor(СсылкаДиалога, РезультатPrecheck);
		ПротоколироватьМетрикуСтадии(СсылкаДиалога, "Execute", НачалоExecute, Ложь, "pre_execution_schema_check", "Execute->Recover");
		Возврат Ложь;
	КонецЕсли;
...
Функция ВыполнитьСтадиюValidate(СсылкаДиалога) Экспорт
	...
	Если ЭтоВосстановимаяОшибка(КодОшибки) Тогда
		...
		Если ЗафиксироватьПовторОшибочногоRunQuery(СсылкаДиалога, DSLДляАнализаПовтора, КодОшибки, 2) Тогда
			...
			ИИА_Сервер.ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, "[LOOP_GUARD] Повторяющийся неуспешный RunQuery: включен флаг принудительной смены стратегии (GetMetadata->GetObjectFields).");
		КонецЕсли;
