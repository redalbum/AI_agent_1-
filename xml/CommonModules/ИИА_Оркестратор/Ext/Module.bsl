...
Функция ВыполнитьСтадиюPlan(СсылкаДиалога) Экспорт
	...
	ИИА_Сервер.СохранитьАртефактПланировщика(СсылкаДиалога, ОтветПланировщика.DSL, "planner_generated", Формат(ТекущаяДата(), "ДФ=yyyyMMddHHmmss"));
...
Функция ПрименитьПолитикуВосстановления(СсылкаДиалога, КодОшибки, СообщениеОшибки) Экспорт
	...
	ИначеЕсли ЛокКод = "field_not_found" Тогда
		...
		Если НЕ ПринятоИсправление Тогда
			...
			Если НЕ ПустаяСтрока(DSLGetObjectFields) Тогда
				ИИА_Сервер.СохранитьАртефактПланировщика(СсылкаДиалога, DSLGetObjectFields, "recovery_field_not_found_getobjectfields", Формат(ТекущаяДата(), "ДФ=yyyyMMddHHmmss"));
				Результат.Обработано = Истина;
	...
	Если Результат.Обработано Тогда
		...
		ПротоколироватьМетрикуСтадии(СсылкаДиалога, "Recover", ТекущаяДата(), Истина, "", "Recover->Plan", Результат.Идентификатор, НомерПопытки + 1, "");
	КонецЕсли;
