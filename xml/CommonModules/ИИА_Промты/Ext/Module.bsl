...
Функция ПолучитьПромптСозданияПлана(ТекстЗадачи, Знач СсылкаДиалога = Неопределено) Экспорт
	...
	"- RETRIEVAL-FIRST: первый RunQuery допустим только после подтверждения объекта/полей (RAG с полями или GetObjectFields). Если подтверждения нет — сначала технический шаг уточнения (GetMetadata/CheckObjectExists/GetObjectFields)." + Символы.ПС +
